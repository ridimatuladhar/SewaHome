...
const TeamAdmin = () => {
  ...
  const fetchTeamMembers = async () => {
    try {
      const response = await fetch('https://stf.org.np/Backend/team/get_team_members.php');
  ...
  const handleDelete = async (memberId, memberName) => {
    if (window.confirm(`Are you sure you want to delete "${memberName}"?`)) {
      try {
        const response = await fetch(`https://stf.org.np/Backend/team/delete_team_member.php`, {
          ...
          {teamMembers.map((member) => (
            <div key={member.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="relative h-48 bg-gray-100">
                <img
                  src={`https://stf.org.np/Backend/team/${member.image}`}
